...
def solve_kkt(U_Q, d, G, A, U_S, rx, rs, rz, ry, dbg=False):
    ...
    if neq > 0:
        h = torch.cat([torch.mv(A, invQ_rx) - ry, torch.mv(G, invQ_rx) + rs/d - rz], 0)
    else:
...
def pre_factor_kkt(Q, G, A):
    ...
    U_Q = torch.potrf(Q)
    U_S = torch.zeros(neq + nineq, neq + nineq).type_as(Q) # partial cholesky of S matrix
    ...
    if neq > 0:
        ...
        try:
            ...
            U11 = torch.Tensor(np.linalg.cholesky(A_invQ_AT.cpu().numpy())).type_as(A_invQ_AT)
